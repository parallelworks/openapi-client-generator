...
Template changes don't need a build step, because templates are `go:embed`ed and read at generate time.
...
### Writing
Applies to commit messages, PR and issue text, code comments, and docs.
- No em dashes or en dashes. Use a comma, colon, parenthesis, or a second sentence.
- This repo has one maintainer. Say what changed and why. Leave out reviewer-facing hedging: apologies for scope, offers to reorganize a PR for someone else, anything addressed to a team that isn't there.
...
- References to current task / fix / callers (`// so foo_test.go can exercise this`, `// matching how auth cookies are built`, `// added for the X flow`) rot. Such context belongs in the PR description.
...
Note: a generated client's exported types/methods are SDK documentation for its consumers, so a concise doc comment there is appropriate. Unexported template helpers (`addQueryParam`, etc.) are internal, so apply the rules above.
...
### Testing
- **Test real code.** Tests import and call the actual functions under test, and never reimplement the logic being tested. For the generator, the strongest tests generate a client from a spec and then compile/run it (`internal/generator/e2e_test.go`), so a broken template fails the test rather than a restated assertion.
